...
function initStarfield() {
  ...
  starFocalLength = starCanvas.width * 1.5;
  starList = [];
...
function drawStarfield() {
  ...
  for (let i = 0; i < NUM_STARS; i++) {
    ...
    const px = (s.x - starCenterX) * (starFocalLength / s.z) + starCenterX;
    const py = (s.y - starCenterY) * (starFocalLength / s.z) + starCenterY;
    if (px < 0 || px >= starCanvas.width || py < 0 || py >= starCanvas.height) continue;
    const r = Math.max(0.3, 1.2 * (starFocalLength / s.z));
    starCtx.fillStyle = s.c + s.o + ')';
    starCtx.beginPath();
